...
template <typename T>
T sinc(T x) {
  if (std::abs(x - 0.0) < 0.000001) return 1;
  ...
  for (int i = 0; i < freqSize; i += 2) {
    ...
    std::transform(b.begin(), b.end(), k.begin(), b.begin(),
                   [m_s, Fi, Fip1, wt2](T b, T k) {
                     return b +
                            (m_s / (4 * std::pow(PI, 2)) *
                             (std::cos(2 * PI * Fip1) - std::cos(2 * PI * Fi)) /
                             (std::pow(k, 2))) *
                                wt2;
                   });
    std::transform(b.begin(), b.end(), k.begin(), b.begin(),
...
void resample(int upFactor, int downFactor, vector<T>& inputSignal,
  ...
  for (int i = 0; i < nz; i++) h.push_back(0.0);
  for (int i = 0; i < coefficientsSize; i++) h.push_back(coefficients[i]);
  int hSize = h.size();
  lengthHalf += nz;
